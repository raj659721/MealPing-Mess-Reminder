      ...
      <div className="flex items-center gap-1 sm:gap-2">
        ...
        <div className="flex items-center gap-1 flex-1">
          ...
          <button
            ...
            className={cn(
              "text-[10px] sm:text-xs font-medium px-1.5 sm:px-2.5 py-1 rounded-full border transition-all w-full text-center",
              lunchTaken
          ...
          >
            {lunchTaken ? t.range.takenLabel : t.range.notTakenLabel}
          </button>
        </div>
        ...
        <div className="flex items-center gap-1 flex-1">
          ...
          >
            {dinnerTaken ? t.range.takenLabel : t.range.notTakenLabel}
          </button>
        </div>
        ...
        <div className="flex items-center gap-1 sm:gap-2 shrink-0">
          <span className="text-xs text-muted-foreground hidden sm:block w-12 text-right">
            {meals > 0 ? `₹${meals * costPerMeal}` : t.common.noData}
          </span>
          <button
            onClick={() => setShowNote((v) => !v)}
            title={t.range.addNote}
            className={cn(
  ...
  return (
    <Layout>
      ...
      <div className="space-y-1 mb-6">
        ...
        <p className="text-muted-foreground text-sm">{t.range.subtitle}</p>
      </div>
      ...
      <Card className="p-5 mb-4 space-y-4">
        ...
        <div className="flex items-center gap-2">
          <span className="text-xs text-muted-foreground">
            {validRange
              ? days.length === 1 ? t.range.daySelected : t.range.daysSelected.replace("{n}", String(days.length))
              : t.common.noData}
          </span>
      ...
      {validRange && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
          {[
            { label: t.range.totalDays, value: stats.days, icon: CalendarRange, color: "text-blue-600", bg: "bg-blue-50 dark:bg-blue-950/20" },
            { label: t.range.mealsTaken, value: stats.mealsTaken, icon: Utensils, color: "text-green-600", bg: "bg-green-50 dark:bg-green-950/20" },
            { label: t.range.mealsSkipped, value: stats.mealsSkipped, icon: CalendarX, color: "text-red-600", bg: "bg-red-50 dark:bg-red-950/20" },
            { label: t.range.totalCost, value: `₹${stats.totalCost}`, icon: IndianRupee, color: "text-orange-600", bg: "bg-orange-50 dark:bg-orange-950/20" },
      ...
      {validRange && (
        <Card className="p-2 sm:p-4 overflow-hidden">
          <div className="flex items-center gap-1 sm:gap-2 pb-2 mb-1 border-b border-border">
            <div className="w-16 sm:w-28 shrink-0">
              <p className="text-[10px] sm:text-xs font-medium text-muted-foreground uppercase tracking-wider">{t.range.tableDate}</p>
            </div>
            <div className="flex-1">
              <p className="text-[10px] sm:text-xs font-medium text-muted-foreground uppercase tracking-wider flex items-center justify-center sm:justify-start gap-1">
                <Coffee className="h-3 w-3 hidden sm:block" /> {t.common.lunch}
              </p>
            </div>
            <div className="flex-1">
              <p className="text-[10px] sm:text-xs font-medium text-muted-foreground uppercase tracking-wider flex items-center justify-center sm:justify-start gap-1">
                <Moon className="h-3 w-3 hidden sm:block" /> {t.common.dinner}
              </p>
            </div>
            <div className="shrink-0 w-6 sm:w-20 text-right flex justify-end">
              <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider hidden sm:block">{t.range.tableCost}</p>
            </div>
          </div>
          ...
          {isLoading ? (
            ...
            <p className="text-sm text-muted-foreground py-8 text-center">{t.range.selectRange}</p>
          ) : (
            <div>
