...
function getWeeklyData(entries: Entry[], year: number, month: number, t: any) {
  const daysInMonth = getDaysInMonth(new Date(year, month - 1));
  const entryMap = new Map(entries.map((e) => [e.date, e]));
  const weeks = [
    { label: t.analytics.week[0], start: 1, end: 7 },
    { label: t.analytics.week[1], start: 8, end: 14 },
    { label: t.analytics.week[2], start: 15, end: 21 },
    { label: t.analytics.week[3], start: 22, end: 28 },
    { label: t.analytics.week[4], start: 29, end: daysInMonth },
  ...
  return weeks.map(({ label, start, end }) => {
    ...
    return { label, [t.common.lunch]: lunch, [t.common.dinner]: dinner };
  });
}

function getDailyData(entries: Entry[], year: number, month: number, t: any) {
  const daysInMonth = getDaysInMonth(new Date(year, month - 1));
  const entryMap = new Map(entries.map((e) => [e.date, e]));
  const data = [];
...
export default function AnalyticsPage() {
  ...
  const weeklyData = useMemo(() => getWeeklyData(entries, year, month, t), [entries, year, month, t]);
  const dailyData  = useMemo(() => getDailyData(entries, year, month, t),  [entries, year, month, t]);
  ...
  const statLabels = [t.analytics.mealsTaken, t.analytics.mealsSkipped, t.common.attendance, t.analytics.totalBill];
  ...
  return (
    <Layout>
      <div className="mb-5">
        ...
        <h1 className="text-2xl font-black text-foreground">{t.analytics.title}</h1>
        <p className="text-muted-foreground text-sm mt-1">
          {format(new Date(year, month - 1), "MMMM yyyy")} — {t.analytics.subtitle}
        </p>
      </div>
      ...
      <Card className="p-5 mb-4 rounded-3xl border-0 shadow-sm">
        <p className="text-sm font-bold text-foreground mb-4">{t.analytics.weeklyBreakdown}</p>
        {isLoading ? <Skeleton className="h-44 rounded-2xl" /> : (
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={weeklyData} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
              ...
              <Bar dataKey={t.common.lunch}  fill="hsl(142 71% 35%)"      radius={[6, 6, 0, 0]} />
              <Bar dataKey={t.common.dinner} fill="hsl(142 71% 35% / 0.4)" radius={[6, 6, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        )}
      </Card>
      ...
      {/* Daily bar chart */}
      <Card className="p-5 mb-4 rounded-3xl border-0 shadow-sm">
        <p className="text-sm font-bold text-foreground mb-4">{t.analytics.dailyMeals}</p>
        {isLoading ? <Skeleton className="h-44 rounded-2xl" /> : (
          <ResponsiveContainer width="100%" height={160}>
            <BarChart data={dailyData} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
              ...
              <Bar dataKey={t.analytics.mealsLabel} radius={[4, 4, 0, 0]} fill="hsl(142 71% 35%)" />
            </BarChart>
          </ResponsiveContainer>
        )}
      </Card>
      ...
      <Card className="p-5 rounded-3xl border-0 shadow-sm">
        <div className="flex items-center gap-2 mb-4">
          ...
          <p className="text-sm font-bold text-foreground">{t.analytics.smartInsights}</p>
        </div>
        {isLoading ? (
