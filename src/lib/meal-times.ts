...
export function saveMealTimes(times: MealTimes): void {
  ...
  window.dispatchEvent(new Event("mealTimesUpdated"));
}

export function useMealTimes(): MealTimes {
  const [times, setTimes] = useState(loadMealTimes);

  useEffect(() => {
    const handleUpdate = () => setTimes(loadMealTimes());
    window.addEventListener("mealTimesUpdated", handleUpdate);
    return () => window.removeEventListener("mealTimesUpdated", handleUpdate);
  }, []);
